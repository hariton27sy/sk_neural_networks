...
lambda_for_policy = 0.3
lambda_for_laplace = 1
...
session_len = 100  # Много ходов для одной сессии не нужно, потому что сессии с большим количеством ходов сильно уменьшают счет
...
class Agent:
    ...
    def update_policy_with_smoothing(self, elite_sessions):
        ...
        self.policy = lambda_for_policy * new_policy + (1 - lambda_for_policy) * self.policy

    def update_policy_with_laplace_smoothing(self, elite_sessions):
        """Сглаживаине по Лапласу"""
        new_policy = np.zeros((self.state_n, self.action_n))
        for session in elite_sessions:
            for state, action in zip(session['states'], session['actions']):
                new_policy[state][action] += 1
        ...
        for state in range(self.state_n):
            new_policy[state] = (new_policy[state] + lambda_for_laplace) / (sum(new_policy[state]) + lambda_for_laplace * self.action_n)
...
def run_episode(episode, env, agent, agent_updating_function, session_len, n_sesions, q_param):
    ...
    if len(elite_sessions) > 0:
        agent_updating_function(elite_sessions)
...
def main():
    ...
    for episode in range(n_episodes):
        run_episode(episode, env, agent, agent.update_policy_with_smoothing, session_len, n_sessions, q_param)
